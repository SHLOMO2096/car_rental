...
- **Styling is mid-migration.** `src/styles/` (`tokens.css` → `base.css` → `components.css`, imported in that order in `main.jsx`) is the new source of truth: CSS custom properties for color/type/space/radius/shadow/z-index, plus `.btn` / `.input` / `.card` / `.alert` classes that carry the `:hover` / `:focus` / `:disabled` states inline styles cannot express. The token values deliberately match the slate/blue hexes already hardcoded across the app, so migrated and unmigrated screens look identical. `pages/Login.jsx` is the reference screen — new work should follow it, and screens should be converted one at a time. Everything else is still inline JS style objects; note that an inline `style` always beats a class, so a screen must drop its style object when it adopts the classes. There is no CSS framework and no dark mode yet (most screens still hardcode their own backgrounds).
