...
- **`frontend/lab.html` + `frontend/scratch/lab.jsx` is the mobile lab** — it renders the real screen components with fixture data inside fixed 360/390/430px frames, and prints an overflow report naming every element that spills past its frame. Run `npx vite --port 5201` then screenshot `/lab.html` with headless Chrome (`chrome --headless=new --window-size=1400,1800 --screenshot=out.png`). It exists because the build, the tests and `audit:ui` all measure structure, not layout: a button can sit 32px outside its dialog with every gate green — which is exactly what shipped. `vite build` only takes `index.html`, so the lab never reaches production; verify that stays true if you touch it. Chrome headless ignores `--window-size` for the layout viewport, which is why the lab frames the widths itself instead of resizing the window.
